...
func (this *CodePager) AppendUnique(name, code string) {
	...
	exist := false
	Mapdo(this.insert_points, func(i interface{}) interface{} {
		kv := i.(string)
		if kv == code {
			exist = true
		}
...
func (this *CodePager) ExportCode(names []string, comment ...string) string {
	...
	blocks := Mapdo(names, func(name interface{}) interface{} {
		return fmt.Sprintf("%s %s block begin\n%s\n%s %s block end\n",
			comment_, name.(string),
			strings.Join(this.lines[name.(string)], this.newline),
			comment_, name.(string))
	})
	return strings.Join(IV2Strings(blocks.([]any)), this.newline)
}
...
func (this *CodePager) TotolLength() int {
	...
	return Mapdo(this.lines, func(vx any) any {
		kv := vx.([]string)
		rv := Mapdo(kv, func(vy any) any {
			return Lenof(vy)
		})
		return rv
	}).(int)
...
func (this *CodeFS) _WriteToDiskFS(bdir string, ext string, fcp *int) error {
	for dir, _ := range this.cfs {
		for file, cp := range this.cfs[dir] {
			*fcp += 1
			fpath := fmt.Sprintf("%s/%s/%s%s", bdir, dir, file, IfElseStr(ext == "", "", "."+ext))
			fdir := path.Dir(fpath)
			if !FileExist(fdir) {
			...
			ErrPrint(err, fpath, cp.AllPoints(), fdir)
		}
	}
	return nil
}
